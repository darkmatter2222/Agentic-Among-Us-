...
| **Agent Memory** | Observations, suspicion levels, conversation history, alibi tracking. **Timestamped history** with relative time formatting (e.g., "30s ago", "2m ago") included in LLM prompts. Memory context shows: recent timeline (last 15 events merged chronologically), last known player locations, suspicion levels with reasons, and alibis claimed. **JSON-formatted context** available for structured LLM responses. |
| **Thought System** | Internal reasoning triggered by events (room entry, spotting agents, etc.). Thoughts shown as cloud-shaped bubble with trailing circles above player. Toggleable via THT button. **Enhanced JSON response format**: thoughts now return structured JSON with optional `suspicionUpdates` (adjust trust levels) and `pendingQuestions` (questions to ask specific players). Fallback to "brain fart" style messages on JSON parse failure. |
...
| **Full Memory Display** | Memory tab in agent profile shows: (1) Stats overview (observations, conversations, accusations, alibis counts), (2) Last known player locations grid with zone and relative time, (3) Unified memory timeline combining all events chronologically with type-colored left borders and icons, (4) Recently heard section with direct-address highlighting. |
| **Enhanced Suspicion Tab** | Suspicion tab shows: (1) Stats overview (high/medium/low/avg counts), (2) Expandable suspicion cards sorted by level with visual bars, (3) Reasons list for each player when expanded, (4) Pending questions section showing questions the agent wants to ask specific players with priority indicators, (5) Collapsible raw suspicion context. |
...
| **Ghost Mode** | GhostSystem.ts manages ghost transitions. Dead players (DEAD state) transition to GHOST state when body is reported or after 30s timeout. Ghosts: (1) Can pass through walls (collision disabled), (2) Get unlimited vision (10x multiplier), (3) Can continue completing tasks, (4) Can only communicate with other ghosts. Ghost sprite: Among Us style floating bean shape with wavy tail, visor, backpack, semi-transparent (alpha 0.45-0.55), bobbing animation (sine wave), alpha pulse effect. |
| **Dead Body Sprite** | Among Us style dead body in AIAgentVisualRenderer.ts showDeadBody(). Uses EXACT same dimensions as living player (16×24 body, 6×14 backpack, 13×9 visor) with rounded rectangles (not ellipses). Cut in half at 60% point: upper half (head/torso with visor and backpack) laying on side, lower half (stump) separated to right. Single white cartoon bone connecting halves with double-ball ends. Blood pool underneath (dark red ellipse). Cyan visor (0x84D2F6) with shine highlight. |
