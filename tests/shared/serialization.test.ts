...
import { serializeAgent, serializeAgentSummary, serializeWorld } from '@shared/engine/serialization.ts';
import { diffWorldSnapshots } from '@shared/engine/stateDiff.ts';
import type { AIAgent } from '@shared/engine/AIAgent.ts';
import type { MovementState } from '@shared/engine/MovementController.ts';
import type { PlayerStateMachineState } from '@shared/engine/PlayerStateMachine.ts';
import { PlayerActivityState, PlayerLocationState } from '@shared/engine/PlayerStateMachine.ts';
...
describe('serialization helpers', () => {
  ...
  function createMockAgent(overrides: Partial<{ movement: MovementState; machine: PlayerStateMachineState; goal: string | null }> = {}): AIAgent {
    const movementState = overrides.movement ?? clone(baseMovementState);
    const machineState = overrides.machine ?? clone(baseStateMachine);
    const goal = overrides.goal ?? 'Test goal';

    const mockMovementController = {
      getState: () => clone(movementState),
    };

    const mockStateMachine = {
      getState: () => clone(machineState),
    };
  ...
  it('serializes agent state with deep copies', () => {
    ...
    const mutableMovement = clone(baseMovementState);
  ...
  it('serializes world snapshots for multiple agents', () => {
    ...
    const agents: AIAgent[] = [
      createMockAgent(),
      createMockAgent({ machine: { ...clone(baseStateMachine), playerId: 'agent-2' } })
    ];
    ...
    expect(new Set(world.agents.map((agent: AgentSnapshot) => agent.id))).toEqual(new Set(['agent-1', 'agent-2']));
  });

  it('serializes agent summaries for lighter payloads', () => {
    const agent = createMockAgent({ goal: null });
    const summary = serializeAgentSummary(agent);
