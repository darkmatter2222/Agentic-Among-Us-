...
class GameRenderer:
    ...
    def render(self, game_state: GameState):
        # Snapshot state for rendering to minimize lock time
        players_snapshot = []
        ...
        with game_state.lock:
            ...
            meeting_caller = game_state.meeting_called_by
            
            # Deep copy needed? No, just shallow copy of PlayerState objects is risky if they are modified.
            # But PlayerState is a dataclass.
            # We need position, name, color, facing, status, is_imposter, is_alive.
            # Let's just copy the list of references and hope attributes aren't modified mid-render in a way that crashes.
            # Actually, position is modified in update_physics.
            # If we read position while it's being written, it might be glitchy but not crash (float atomic-ish).
            # But iterating the dict is dangerous.
            # So we copy the values.
            players_snapshot = list(game_state.players.values())
            
            # We also need to know about map, but map is static.
        ...
        # Draw Players (using snapshot)
        self._draw_players(players_snapshot, game_state.map)
        
        # Draw UI Overlay
        self._draw_ui(game_state.current_turn, phase)
        
        # Draw Meeting Overlay
        if phase == GamePhase.MEETING:
            self._draw_meeting_overlay(meeting_caller)
        
        # Draw Sidebar
        self._draw_sidebar(players_snapshot)
    ...
    def _draw_players(self, players: list[PlayerState], game_map: SpaceshipMap):
        from game.vision import VisionSystem
        
        # OPTIMIZATION: Only draw detailed vision for first player (or none for performance)
        # Drawing vision for all 8 players causes severe performance issues
        draw_vision_for_first_only = True
        ...
        for idx, player in enumerate(players):
            ...
            # --- 1. Draw Vision Cone (FOV) - OPTIMIZED ---
            # Only draw for first player to avoid expensive raycasting
            if draw_vision_for_first_only and idx == 0:
                # Raycast vision
                vision_poly_world = VisionSystem.cast_vision_rays(player, game_map)
            ...
            # --- 2. Draw Action Radius (Optional - skip for performance) ---
            # Uncomment to show action radius
    ...
    def _draw_sidebar(self, players: list[PlayerState]):
        # Sidebar background
        ...
        for player in players:
            ...
            # Thinking state
            status_text_str = f"{status} - {player.status_message}"
            
            details_color = (180, 180, 180) if player.is_alive else (80, 80, 80)
            details_surf = self.font.render(status_text_str, True, details_color)
            self.screen.blit(details_surf, (sidebar_rect.left + 35, y_offset + 16))
            
            y_offset += 40

    def _draw_ui(self, current_turn: int, phase: GamePhase):
        # Draw turn info
        info = f"Turn: {current_turn} | Phase: {phase.value}"
        text = self.large_font.render(info, True, (255, 255, 255))
        self.screen.blit(text, (10, 10))

    def _draw_meeting_overlay(self, meeting_called_by: str):
        """Draw a big red overlay for meetings."""
        overlay = pygame.Surface((self.width - self.sidebar_width, self.height), pygame.SRCALPHA)
        overlay.fill((200, 0, 0, 100)) # Red semi-transparent
        self.screen.blit(overlay, (0, 0))
        
        # Text
        text = self.large_font.render("EMERGENCY MEETING", True, (255, 255, 255))
        text_rect = text.get_rect(center=((self.width - self.sidebar_width)//2, self.height//2 - 50))
        self.screen.blit(text, text_rect)
        
        if meeting_called_by:
            sub_text = self.font.render(f"Called by {meeting_called_by}", True, (255, 255, 255))
            sub_rect = sub_text.get_rect(center=((self.width - self.sidebar_width)//2, self.height//2 + 10))
